      ...
      {products.map(product => {
        return (
          <ProductGridSingle
            ...
            cartItem={cartItems.filter(cartItem => cartItem.prdNo === product.prdNo)[0]}
            wishlistItem={
              wishlistItems.filter(wishlistItem => wishlistItem.prdNo === product.prdNo)[0]
            }
            key={product.prdNo}
          />
        )
...
const mapStateToProps = (state, ownProps) => {
  return {
    products: getProducts(ownProps.category, ownProps.type, ownProps.limit),
