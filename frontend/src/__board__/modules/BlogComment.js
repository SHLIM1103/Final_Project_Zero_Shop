                    ...
                    {localStorage.getItem("token") != null ? (
                      <>
                        {JSON.parse(localStorage.getItem("user")).usrNo === r.usrNo ? (
                          <>
                            <button>
                              <Link to={process.env.PUBLIC_URL + `/comment-update/${r.rplNo}`}>
                                수정
                              </Link>
                            </button>
                            <button onClick={remove1}>
                              <Link to={localStorage.setItem("rpl", r.rplNo)}>삭제</Link>
                            </button>
                          </>
                        ) : (
                          ""
                ...
                <div className="col-md-12">
                  <div className="text-leave">
                    <textarea
                      ...
                      name="rplContent"
                      defaultValue={""}
                      onChange={e => setRplContent(`${e.target.value}`)}
                    />
